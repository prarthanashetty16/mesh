...
const createTask = async (taskData) => {
  ...
  console.log(`[TaskModel] Inserting task into database: "${title}"`);
  ...
  const result = await executeQuery(query, [
    title,
    description || null,
    parseFloat(price) || 0, // Ensure price is a number
    deadline || null,
    created_by,
    area_id || null,
    latitude,
  ...
  if (!result || !result.insertId) {
    console.warn('[TaskModel] Warning: Insert successful but no insertId returned');
  }
 ...
 */
const getTasksByCreatorCount = async (userId) => {
...
const getNearbyTasks = async (latitude, longitude, radiusKm = 10, limit = 10, offset = 0) => {
  ...
  return await getAll(query, [latitude, latitude, longitude, latitude, latitude, longitude, radiusKm]);
};
