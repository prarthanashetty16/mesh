...
const createNewTask = async (req, res, next) => {
  try {
    ...
    console.log(`[TaskController] Creating new task for user ${userId}:`, { title, price });
    ...
    console.log(`[TaskController] Task created successfully with ID: ${taskId}`);
    ...
    console.error('[TaskController] Error in createNewTask:', error.message);
    next(error);
  }
};
...
const getMyTasks = async (req, res, next) => {
  try {
    ...
    const totalCount = await getTasksByCreatorCount(userId);
